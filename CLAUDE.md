...
- `docs/home-widgets-spec.md` — **home-screen widget (Android)**: the snapshot it renders, why *connecting* goes through the app's own code (live entitlement check) while *disconnecting* is native. §9 records why the **iOS widget was removed on 2026-08-01** — what Apple actually documents about which process a widget App Intent runs in, why no marker could have launched a terminated app on iOS 17, and the cheap checks to run first if it is ever revived. Sections 4 and parts of 5–8 describe deleted iOS code and are kept as history
- `docs/store-compliance.md` — **what App Store and Google Play require (2026-07-31)**: one canonical table of what the app does with data, and every store-side form that has to repeat it — Apple's 5.4 and App Privacy, Play's VPN declaration, Data safety and prominent disclosure — each marked done or not. Its companion files are `backend/legal/privacy.{html,ru.html}` (the hosted policy, served by Caddy at `/privacy` and `/privacy/ru`) and the two `PrivacyInfo.xcprivacy` (Runner and PacketTunnel — the widget's went with the widget on 2026-08-01). Read it before touching any of them: the rule it exists for is that policy, store form and code must give the same answer, and a divergence is its own rejection
