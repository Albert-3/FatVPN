...
> **Обновление 2026-08-03.** Цифры выше — историческая запись на 2026-07-28. Сейчас
> `flutter test` даёт **293 прошло / 1 пропущен** в `app/` и **213** в плагине, analyze
> чист. И строка «на устройстве не проверено ничего» больше не верна целиком: прогон на
> эмуляторе 2026-08-02 закрыл TA1–TA4 и TA8–TA13 (сплит-туннель доказан измерением IP,
> не UI), кнопка виджета проверена на устройстве 2026-08-03, а часы сессии и страна
> (находка **N3**) — там же. Не проверены на устройстве по-прежнему IPv6/MTU
> (TA14–TA16) и пиннинг (TA17–TA19).
...
| 1.2 Certificate pinning | 🟡 сделано 2026-08-02 (`d32945a`) | Пин **на CA, а не на ключ сервера**: `SecurityContext(withTrustedRoots: false)` + четыре корня ISRG (`lib/config/ca_pins.dart`), через него строится единственный HTTP-клиент приложения. Разбор и операционный контракт — в тексте находки ниже. На устройстве не проверено: **TA17 блокирует выпуск** любой сборки с этой правкой |
...
✅ закрыт 2026-07-30 (`https://api.fatklyuchi.space`). Из §5 не закрыто ничего кодом;
осталась приёмка на устройстве (TA14–TA19) и выключение старого HTTP-порта, когда
...
#### 🟠 N3. Приложение врало о туннеле, который подняло не оно (2026-08-03, `3c5e487`)
Найдено прогоном на эмуляторе, обе половины — про один и тот же слепой угол: аудит
проверял состояния, которые приложение создаёт **само**, а виджет и уведомление умеют
поднимать и опускать туннель мимо него.

**Часы сессии считали выключенный VPN.** `_sessionStartedAt` персистится, а очищался
только в `disconnect(endSession: true)` — то есть при выключении **из приложения**.
Любой другой конец туннеля (кнопка виджета, «Стоп» в уведомлении, убитый процесс,
перезагрузка) оставлял старт на диске, а `_trackSessionStart` писал только в `null` —
следующее подключение наследовало чужое начало: туннель, поднятый тридцать секунд назад,
показывал `09:34:33`. Уведомление самого туннельного сервиса всё это время показывало
верное время, и это же выдало, что врёт именно приложение.

**Фикс:** новая сессия якорится там, где поднимается туннель, **если** свежий teardown не
объявил, что это подключение продолжает прежнюю. Объявление сделано **дедлайном, а не
флагом**: авто-переключение и переподключение из настроек идут за своим
`disconnect(endSession: false)` в пределах секунд, а флаг, который никто не снимает,
тихо сохранил бы часы и через сутки. Ошибиться в эту сторону — обнулить часы там, где
их можно было продолжить; в другую — считать часы простоя.

**И страна.** Перезапуск поверх живого туннеля (обычный финал подключения с виджета)
оставлял `connectedNode == null`, и карточка показывала «Подключено к Лучший сервер»
при явном подключении к конкретной ноде. Нода теперь персистится и восстанавливается,
причём восстановление **само** дёргает слушателей: сверка вокруг него уведомляет только
при смене *состояния*, а оно к этому моменту обычно уже не меняется — без этого нода
доезжала до карточки минутами позже, при первом постороннем событии, что выглядит ровно
как неисправленный баг.

**Проверено на устройстве** (единственная правка этой недели, про которую так можно
сказать): после перезагрузки со стартом от 09:04 на диске подключение показало
`00:00:30`; подключение с виджета при мёртвом приложении дало в логе
`Restored connected node "pol_4vps_tcp"` и Польшу на карточке. Приёмка — TA20/TA21.
Тестом **не** покрыто одно: что восстановление уведомляет. Против фейка плагина
состояние меняется всегда, поэтому такой тест зеленеет и с убранным фиксом — зелёный
тест, который не может упасть, читается как гарантия и ею не является.
...
1. **HTTPS + удаление `usesCleartextTraffic`** — `api_config.dart:4`,
   `AndroidManifest.xml:16` (§1.1) — ✅ закрыт 2026-07-30: приложение ходит на
   `https://api.fatklyuchi.space`. Исключение для `87.121.221.229` в
   `network_security_config.xml` держится намеренно — там ещё сидят разошедшиеся сборки
...
11. Certificate pinning (§1.2) — ✅ 2026-08-02 (`d32945a`), пин на CA; приёмка TA17–TA19
