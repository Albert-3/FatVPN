...
> ⚠️ **Пометка «рабочее дерево» устарела и читается неверно — 2026-08-02.** На момент
> написания таблицы правки лежали незакоммиченными; в тот же день они доехали до `master`
> (`d8cec32`, `6813852`, `a961603`, `3843afd`, `be2bd80`, `fcf90fa`, `5bca5f0`, `668202a`,
> `165de3c`, `6bd92e1`, `c66b839` и соседние). Проверено 2026-08-02 точечным поиском по
> закоммиченному дереву (`git grep … HEAD`) для 17 характерных находок — 1.5, 1.6, 1.7,
> 1.9, 1.10, 2.2, 2.4, 2.5, 2.12, 2.13, 2.15, 3.1, 3.2, 3.4, 3.5, 3.10, 3.12 — все на
> месте. **Ничего не потеряно.** Строки таблицы оставлены как есть: это запись того, как
> было, а не утверждение о сегодняшнем дне. Закрывает 6.1 в `docs/open-bugs.md`.
...
### 🟠 1.2 Полностью отсутствует certificate pinning — 🟡 Сделано в коде 2026-08-02, на устройстве не проверено
...
> **Как сделано (2026-08-02).** Не пином ключа, а сужением якорей доверия:
> `SecurityContext(withTrustedRoots: false)` плюс четыре корня ISRG
> (`app/lib/config/ca_pins.dart`) — то есть системного хранилища у клиента больше
> нет вовсе. Точка входа одна: `createBffHttpClient()` в
> `lib/services/pinned_http_client.dart`, её зовёт конструктор `ApiClient`, а
> другого HTTP-клиента в приложении нет.
>
> **Почему не SPKI-пин листа, как предлагала находка.** «Два ключа + механизм
> обновления» на этой площадке недостижимы: сертификат выпускает Caddy сам, и
> certmagic генерирует **новый ключ на каждое продление** (~60 дней). Пин ключа
> означал бы кирпич у всех через два месяца, а механизм обновления пина — это
> релиз в магазин, то есть дни на iOS. Backup-ключа, который можно было бы
> заготовить, у нас тоже нет: мы не владеем выпуском. Якорь меняется раз в годы и
> с публичным уведомлением, а все четыре текущих корня лежат в наборе — продление,
> смена промежуточного и переход RSA↔ECDSA проходят молча.
>
> **Проверено (не на устройстве):** живой `api.fatklyuchi.space` отвечает 200 под
> этим набором корней тем же стеком Dart/BoringSSL, что у приложения, а
> `google.com` под ним же — `HandshakeException`. Офлайн: `test/ca_pinning_test.dart`
> поднимает настоящий TLS-сервер с чужим CA и требует отказа, плюс сверяет
> отпечатки корней с опубликованными.
>
> ⛔ **Контракт:** смена УЦ у домена кладёт все установленные сборки. Новый корень
> — сначала в приложение и в раздачу, потом на сервер. Приёмка — TA17–TA19
> (`docs/release-test-checklist.md` §2f).
...
>
> **Порт закрыт отдельно, 2026-08-02.** Свободный порт запрашивается у ОС на каждый старт
> (`VpnController._pickClashApiPort()` → `ServerSocket.bind(loopbackIPv4, 0)`) и уходит в
> `external_controller`; `VpnTunnelHealthProbe.kt` его и так читал из конфига. Сохраняется
> под `vpn_clash_api_port` рядом с секретом и по той же причине — приложение может быть
> перезапущено поверх живого туннеля; нет записи ⇒ туннель поднят прошлой сборкой ⇒ опрос
> на 16756. Теперь чужое приложение не знает, куда стучаться, — раньше оно получало `401`
> и по одному этому ответу узнавало, что VPN работает. Переход на `CommandClient` без
> TCP-порта по-прежнему не рассматривался. Подробности — `docs/open-bugs.md` 3.4.
