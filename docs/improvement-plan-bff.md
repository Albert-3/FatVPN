...
| **S10 — размер ответа панели** | Вход ограничен (64 КБ тело, `StringLength`, 500 id на вызов), но `MaxResponseContentBufferSize` для ответов Remnawave не задан |
...
> друг друга по кругу.
> **Что осталось:** включить флаг, поменять `bffBaseUrl` в приложении и выставить
> `AllowedHosts` — всё это упирается в HTTPS-домен, которого пока нет. Это **общий
> блокер релиза** с приложением (§1.1) и iOS (`NSAppTransportSecurity`).
...
### 🟡 S10. DoS-векторы по размеру пейлоада — 🟡 Частично (`820b1fe`)
...
> **Что осталось:** `MaxResponseContentBufferSize` для ответов панели не задан —
> зависшая/враждебная панель всё ещё может отдать сколь угодно большой `/sub/...`.
...
5. `MaxResponseContentBufferSize` на ответы панели (S10, остаток).
