...
> **Что этот экран показывает, когда туннель подняло не приложение** (2026-08-03,
> `3c5e487`). Туннель умеет подниматься и опускаться мимо UI — кнопкой виджета,
> «Стоп» в уведомлении, перезагрузкой. Правила такие: **таймер отсчитывает от
> подъёма туннеля**, а не от последнего запуска приложения, и не переживает
> выключение VPN (исключение — переподключение в пределах секунд: авто-переключение
> и смена настроек продолжают ту же сессию); **страна на плашке восстанавливается**
> при запуске поверх живого туннеля, а не откатывается на «Лучший сервер».
> Плитка на домашнем экране — отдельный документ, `docs/home-widgets-spec.md`.
...
- **CONNECTION SETTINGS**:
  ...
  - Тумблер «Защита от трекинга» (2026-08-02) — блокировка трекерных и
    рекламных доменов внутри туннеля
  - Тумблеры «Переподключаться автоматически» и «Блокировать трафик без VPN» —
    в исходных макетах их не было, добавлены при реализации
- **ROUTING**:
  - Тумблер «Раздельное туннелирование» — мастер-переключатель. Переехал сюда
    из шапки экрана раздельного туннелирования 2026-08-02 (п.5): включение —
    то, ради чего в настройки и заходят, и было единственным, ради чего
    приходилось проваливаться на экран глубже
  - «Split tunneling settings» — «Choose apps that bypass the VPN» →
    отдельный экран (см. п.5)

> Тумблер защиты от трекинга и тумблер раздельного туннелирования
> **взаимоисключающие**: включение одного гасит другое и возвращает при
> выключении. Подпись под выключенным объясняет, почему он выключен.
- **SYSTEM**:
  - Language → «English»
- **LOGS MANAGEMENT**:
...
- Заголовок «Split tunneling». ⚠️ Тумблер вкл/выкл был в шапке по макету; с
  2026-08-02 его там **нет** — он в карточке ROUTING на экране настроек (п.4).
  В шапке остались только стрелка «назад» и заголовок
