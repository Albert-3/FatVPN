...
**Деплой (порядок обязателен — сначала BFF):**

```bash
# 1. BFF: закоммитить и запушить master, затем
cd /opt/fatvpn-bff/backend && git pull && docker compose build --no-cache bff && docker compose up -d bff
# (миграция LinkTokenToAccount накатится сама на старте)

# 2. Бот
cd /opt/FatVPN && docker compose build --no-cache && docker compose up -d --force-recreate
```

Старый BFF молча игнорирует неизвестные поля JSON, так что бот, задеплоенный первым, отработает
без ошибок — просто `telegramUserId` не сохранится и коды останутся непривязанными.

⚠️ **Не проверено ничего, кроме тестов** (BFF 113 passed / 7 skipped, приложение 190 passed,
`pairing_state` — отдельная проверка на однократность/границы/TTL). Оба контейнера на сервере
по-прежнему работают на старых образах. Прогон по чек-листу из «Доработки 2» — после деплоя.
