...
> Снято 2026-08-02: рабочее дерево чистое, всё это в `master`. Осталась только
> обычная зависимость — новая сборка на устройстве.
...
> Снято частично 2026-07-30: чек-лист прогнан руками, нашлось и починено 5 багов (в том
> числе кнопка старого меню подключала не тот ключ). **Не проверены** «поменять ключ»,
> продление и триал — на тот момент был мёртв токен бота к панели.
...
✅ **Проверено на устройстве заказчиком (2026-08-01)** на TestFlight **189**: тот самый сценарий
— триал истёк, ключ выбран в боте, возврат в приложение — работает. Это первый факт о
*поведении* iOS-сборки со времён build 18: всё, что вышло между ними, доказывало только
собираемость. Раскатано также: TestFlight **188** (промежуточная, лишь первая правка), Android
APK и AAB собраны локально (`b56cbeb`, подпись `CN=FatVPN`, только arm64/arm32).

⚠️ Что **не** проверялось отдельно: зеркальный случай — начать pairing, не доводить его,
вставить ключ вручную; приложение должно остаться на вставленном ключе, даже если бот потом
завершит код. И возврат неиспользованной сессии через `/auth/logout` (виден только по тому,
что слот устройства освободился).

## Защита от трекинга + переезд мастер-переключателя сплита (2026-08-02)

Заказчик попросил «блокировщик рекламы»: тумблер в настройках, который можно включить и при
необходимости выключить, — и отдельно спросил, будет ли видно в админке, сколько человек им
пользуется.

**Ответ про админку: нет и само не появится.** Ни в приложении, ни в BFF нет никакой
телеметрии — ни событий, ни счётчиков, ни SDK аналитики. Настройки (DNS, сплит, kill switch)
живут только на телефоне. BFF знает про устройство ровно то, что нужно для работы ключа:
солёный хэш `attestationToken`, платформу, слот и `LastSeenAt`. Число установок видно в
кабинетах магазинов; «сколько включило функцию» потребует осознанно добавленного сбора
статистики, а он тянет за собой правки в политике и в анкетах Play Data safety / Apple App
Privacy. Решение отложено до подготовки публикации.

### Как сделана блокировка

`app/lib/services/tracker_block_list.dart` — 250 доменных суффиксов, вшитых в сборку;
`RouteOptions.blockedDomainSuffixes` (новое поле плагина) превращается в одно правило
`domain_suffix → block` в `SingboxRouteRulesBuilder`. Настройка — `conn_block_trackers`,
по умолчанию **выключена**.

**Почему список вшит, а не качается.** Удалённый `rule_set` в sing-box — зависимость *старта
роутера*: скачивание идёт до того, как поднялся туннель, из сети, которая и есть причина, по
которой пользователь поставил VPN. Провал = «включил защиту, VPN перестал подключаться». На
iOS вдобавок выключен `cache_file` (`AdvancedOptions.memoryLimit`, лимит памяти расширения),
так что список качался бы на **каждом** подключении. Цена решения: список меняется только
вместе со сборкой. Заготовка `SingboxRuleSet` с `type: remote` в плагине осталась
неиспользованной — если понадобится авто-обновление, безопасный путь другой: качать в фоне
из Dart в файл и никогда не блокировать этим старт туннеля.

**Порядок правил — это и есть функция.** Правило блокировки стоит **выше** правил `direct`:
`yandex.ru` попадает в список обхода автоматически (сид-батчи), а `mc.yandex.ru` и
`an.yandex.ru` — его поддомены, и правило обхода перехватило бы их первым. Конфиг при этом
выглядел бы совершенно правильным. Гарантия закреплена в
`packages/singbox_mm/test/tracker_block_rule_order_test.dart` (7 тестов) — на уровне плагина,
потому что приложение такую комбинацию больше не собирает (см. взаимоисключение ниже), а
генератор общего назначения обязан быть прав независимо от вызывающего.

Вместе с блокировкой включается сниффинг (`resolveDestination`): доменные правила иначе не
на чем матчить — fake-IP закрывает только `A`-запросы.

### Название: «Защита от трекинга», не «Блокировка рекламы»

Google Play, **Device and Network Abuse**, запрещает приложения, вмешивающиеся в показ рекламы
в чужих приложениях; по этой формулировке из Play удаляли Adblock Plus и Blokada. VPN с
фильтрацией в Play живут, когда фильтрация подаётся как защита от трекинга (у AdGuard в Play
лежит VPN, а блокировщик — нет). Переименование сквозное, до имён в бинарнике, потому что
автопроверка Play читает содержимое APK: `TrackerBlockList`, `trackerProtection`,
`conn_block_trackers`. Подзаголовок в приложении при этом честно говорит, что рекламные сети
тоже блокируются — врать пользователю ради магазина хуже, чем снимаемый этим риск. Главный
остаток риска в **тексте листинга**, а не в коде. Развёрнуто: `docs/store-compliance.md` §3.1a.

### Взаимоисключение с раздельным туннелированием (решение заказчика)

Включаешь защиту — раздельное туннелирование выключается; выключаешь — **возвращается** со
всеми правилами и режимом. Симметрично в обе стороны, из обеих точек входа. Маркер
`conn_exclusive_suspended` персистится, иначе «включил, закрыл приложение, через день
выключил» теряло бы правила молча. Возвращается только то, что приложение выключило само.

Это убирает вопрос, на который иначе пришлось бы отвечать: с обоими включёнными домен из
списка обхода всё равно мог бы блокироваться, то есть список обхода тихо перестал бы значить
то, что значил во всех выпущенных сборках.

⚠️ **Главный риск пары — на Android.** Исключённые из VPN приложения (обычно банк) при
включении защиты оказываются **внутри** туннеля. Для пользователя это выглядит как «банк
сломался». Единственное, что это объясняет, — подпись под выключенным переключателем, и её
видимость надо проверить на устройстве отдельно (TA8).

Состояние «оба включены» на диске (ручная правка, откат на старую сборку и обратно) чинится
при загрузке: побеждает раздельное туннелирование, в лог идёт предупреждение.

### Мастер-переключатель сплита переехал в настройки

Был в шапке экрана раздельного туннелирования (`SplitTunnelHeader`, общий для Android и iOS),
стал строкой в карточке «Маршрутизация» над ссылкой на тот экран. Из шапки убран, не
продублирован. Четыре строки локализации говорили «включите переключатель выше» — переписаны
на «включите его в настройках»: подсказка, ссылающаяся на несуществующий элемент, хуже
отсутствия подсказки.

### Тесты и состояние

277 тестов приложения, 212 плагина, `flutter analyze` чист на обоих. Приёмка —
`docs/release-test-checklist.md` §2d, 12 пунктов (TA1–TA13); TA13 закрывается глазами, потому
что `SettingsScreen` для рендера в тесте требует живую сессию и мок сети.

⚠️ `audit_best_node_test.dart` («candidates are measured concurrently») мигает на медленной
машине: упал в одном полном прогоне и при запуске в одиночку, прошёл в следующем полном —
одинаково с этими изменениями и без них. Он меряет перекрытие по часам; один красный прогон
считать шумом.

### Прогон на эмуляторе (2026-08-02)

Pixel 7 / API 35, живая триальная сессия с прод-BFF, туннель на реальные ноды. Маршрутизацию
смотрели не в UI, а в `files/singbox/active-config.json` работающего туннеля
(`adb shell run-as`) и по поведению браузера через него. **TA1–TA4, TA8–TA13 прошли**, TA5 и
TA7 частично. Полная таблица — в чек-листе.

Что дал прогон сверх галочек:

- **Сплит-туннель доказан измерением.** Chrome в списке обхода показал IP хоста
  (37.252.72.138, совпал с IP машины), после включения защиты — 130.49.218.80, выход VPN.
  То есть риск из ⚠️ выше не гипотетический: исключённое приложение действительно уезжает
  внутрь туннеля. Подпись под переключателем при этом видна и читается как объяснение.
- ⛔ **В блок-листе не было `yabs.yandex.ru`** — хоста, с которого Яндекс Директ отдаёт блоки.
  С включённой защитой `ya.ru` продолжал рисовать баннер: `an.yandex.ru` в списке закрывает
  старый путь доставки, не текущий. На единственной странице, где русский пользователь скорее
  всего и проверит тумблер, тумблер выглядел неработающим. Добавлен.
- ⛔ **`AppLogger.e` падал на каждой ошибке.** `IOSink.flush()` связывает sink до завершения
  своего future; `e()` пишет две строки, и первая запускает flush по ошибке, так что вторая
  (стек) всегда попадала в это окно и получала
  `Bad state: StreamSink is bound to a stream`. `main()` зовёт `log.e` из
  `FlutterError.onError` — значит бросок происходил **внутри обработчика ошибок**: сама ошибка
  не доходила ни до консоли, ни до файла, ни до саппорт-бандла, а кадр отбрасывался.
  Проявлялось как **пустой экран раздельного туннелирования** — список приложений не рисовался
  вовсе, и в логе не было ни слова почему. Под ним пряталась безобидная debug-ассерта
  `ListTile background color or ink splashes may be invisible` (осталась, не трогали).
  Починено и закреплено `app/test/app_logger_error_flush_test.dart` — три теста, все три
  падают на старом коде.

Чего эмулятор не проверяет и не мог: смену сети, doze, отзыв разрешения VPN системой,
рекламные сети вне Яндекса и TA5 по приложениям (бот, карты, Ozon/WB, мессенджеры).

## Покупка ключа в боте не выдавала код для приложения (2026-08-03)

**Симптом от заказчика:** купил ключ в боте — экран «🔥 · Доступ успешно получен» показывает
ссылку на подписку и кнопки платформ, и всё. Кода, который вставляют в приложение, нет.

**Причина оказалась глубже симптома: покупка вообще не ходила в BFF.** Не «код не нарисовали»,
а `handle_tarif_purchase` (`bot/handlers/callback_handlers.py`) после успешной оплаты не звал ни
`register_short_token`, ни `upsert_subscription`. То есть кода не существовало в природе, и BFF
не знал, что у аккаунта появился ключ. Соседние ветки это делают давно: `handle_key_details`
(экран «Ваш ключ» из «Мои ключи») и `handle_change_key` («Поменять ключ») — обе выпускают код и
синхронизируют подписку. Покупка была единственным входом, который этого не делал, — то есть
ровно на пути «заплатил и хочу пользоваться» приложения как бы не существовало, а найти код
можно было только зайдя потом в «Мои ключи».

**Что сделано.** В `handle_tarif_purchase` после успешной покупки:

1. срок берётся из панели (`gettime(full_key)`), а не «сегодня + дни» — если покупка продлила
   существующий ключ, реальная дата окончания больше; фолбэк на «сегодня + дни» остался;
2. `register_short_token(short_uuid, expires_at_ms, user_id)` — код для приложения;
3. код кладётся в `key_handler.key_token_map[full_key]`, чтобы экран ключа показал **этот же**
   код, а не выпустил ещё один (`register_short_token` каждый раз минтит новый);
4. `upsert_subscription(..., key_code=short_token)` **без** `make_active` — сознательно: BFF
   применит это к аккаунту без ключа или с уже истёкшим, то есть к обычному случаю «подписка
   кончилась → купил новую», а переезд с живого ключа на другой остаётся явным действием
   пользователя. Если заказчик захочет «покупка всегда переключает приложение» — это одно слово
   `make_active=True`, но это продуктовое решение, а не техническое;
5. весь блок в `try` — деньги уже списаны и ключ выпущен, поэтому ни одна ошибка здесь не имеет
   права оставить пользователя без экрана: при сбое рисуется прежний экран, просто без кода.

**Проверено на живом стеке (2026-08-03), но не покупкой.** Из контейнера бота вызваны те же две
функции с синтетическим `telegramUserId` — код выпустился, в `Accounts` появилась строка с
`CurrentSubscriptionId` и `CurrentKeyCode`, в `Tokens` — код с `AccountId`; тестовые строки затем
удалены. То есть путь бот → BFF → БД рабочий целиком; непроверенным остаётся сам экран после
реальной оплаты (нужен баланс в боте). Смотреть при первой же покупке:
`docker logs -f fatvpn-bot | grep -i "код приложения"` — эта строка появляется только когда
выдача кода упала.

**Развёрнуто:** `docker compose build --no-cache && up -d --force-recreate` в `/opt/FatVPN`,
бот поднялся и опрашивает Telegram. Оригинал — `callback_handlers.py.pre-buycode` рядом с файлом.

**Пробный ключ — то же самое, сделано следом (2026-08-03).** Экран «✅ · Пробный ключ создан»
(`bot/handlers/trial_handler.py`) отдавал только ссылку, то есть человек брал пробу в боте и в
наше собственное приложение попасть не мог. Правка ровно та же: срок из панели с фолбэком на те
же выданные дни (литерал `2` заодно стал `trial_days` и не дублируется), код, кэш и
`upsert_subscription` без `make_active`, весь блок в `try`. Плюс одна строка проводки в
`main_refactored.py` — `trial_handler.key_handler = key_handler`, рядом с такой же для
`callback_handler`: без неё «Мои ключи» выпустили бы для этого ключа второй код.

Триал приложения (`POST /trial` в BFF, со своей защитой от переустановок) к этому отношения не
имеет — это разные вещи с похожим названием: здесь речь о ключе панели, выданном ботом.

**Проверено после сборки:** контейнер поднялся и опрашивает Telegram, в образе лежит и проводка,
и выдача кода, а `getem`/`gettime` действительно видны в обоих модулях — они приходят звёздочным
импортом (`from database.db import *`), то есть отсутствие имени вылезло бы не при старте, а
только у пользователя, взявшего пробный ключ. Сам экран после реального нажатия не проверялся.
Оригиналы — `trial_handler.py.pre-buycode` и `main_refactored.py.pre-buycode`.

## Что ещё доехало 2026-08-02…03 (сводка со ссылками)

Эти работы живут в своих документах — здесь только запись, что они случились, чтобы журнал
проекта не расходился с деревом.

**Транспорт до BFF закрыт с обеих сторон.**
- **Пин на удостоверяющий центр** (`d32945a`, 2026-08-02): единственный HTTP-клиент приложения
  строится на `SecurityContext(withTrustedRoots: false)` с четырьмя корнями ISRG
  (`app/lib/config/ca_pins.dart`), то есть ~150 CA устройства и любой CA из профиля
  конфигурации на iOS больше не могут поручиться за `api.fatklyuchi.space`. Пин **на ключ
  сервера отвергнут сознательно**: certmagic у Caddy минтит новый ключ на каждое продление
  (~60 дней), выключателя у плохого пина нет, а починка — релиз в сторах. ⛔ Цена: смена CA
  домена кладёт **все** установленные сборки; новый корень едет в приложение первым, сервер —
  вторым. Разбор — `docs/improvement-plan-app-android.md` §1.2, приёмка TA17–TA19.
- **`AllowedHosts` на проде** (`aaffa5e` + деплой 2026-08-03): сервер отвечает только на имена
  из списка, всё прочее — 400. Там же ограничен ответ панели (4 МБ → 502), чем закрыт
  последний открытый пункт BFF-аудита S10. Контракт — `docs/api-contract.md`.

**IPv6 на Android** (`d642b0b`, 2026-08-02): у TUN появился ULA-адрес — без адреса семейства
ОС не отдаёт в туннель ни одного v6-пакета, и правило `::/0 → block` не видит ничего; вместе
с ним MTU 1100 → 1280, потому что ниже минимального канального MTU для IPv6 интерфейс не
может нести inet6-адрес вовсе. ⚠️ Утечку никто не *видел* — это вывод из слепков конфига, а
не измерение; на устройстве не проверено (TA14–TA16).

**Часы сессии и страна** (`3c5e487`, 2026-08-03): время сессии считало и то время, когда VPN
был выключен (любой конец туннеля мимо приложения — виджет, «Стоп» в уведомлении, убитый
процесс, перезагрузка — оставлял старт на диске), а перезапуск поверх живого туннеля показывал
«Подключено к Лучший сервер» при явном подключении к ноде. Обе половины про один слепой угол:
раздел «Синхронизация состояния при переоткрытии приложения» выше чинил ровно эту область, но
тогда туннель поднимало и опускало только само приложение. **Проверено на устройстве**,
приёмка TA20/TA21; разбор — находка **N3** в `docs/improvement-plan-app-android.md`.

**Виджет на домашнем экране** — свой документ, `docs/home-widgets-spec.md`. Коротко: кнопка на
Android впервые нажата на устройстве 2026-08-03 и работает в обе стороны, включая нажатие при
выгруженном приложении; на iOS App Intent удалён целиком (решение заказчика, `a3eee9d`) —
нажатие стало ссылкой `fatvpn://widget/toggle` на всех версиях iOS, фонового переключения нет,
на устройстве эта правка не проверялась.
