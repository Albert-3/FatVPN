...
⚠️ **Тот же пробел рядом, не тронут:** экран «✅ · Пробный ключ создан»
(`bot/handlers/trial_handler.py`) тоже отдаёт только ссылку на подписку. Это отдельное решение:
у приложения есть собственный триал со своей защитой от переустановок, и выдавать ключ бота
в приложение — продуктовый выбор, а не очевидная симметрия. Правка та же и займёт пять строк.
