...
выше), а отдельный хост в панели — `🌍 Белые списки #1`, `vless://…@81.222.127.189:443?type=xhttp&
path=/api/v1/assets&security=tls`. Он держит связь во время отключений мобильного интернета: адрес,
который он фронтит, лежит в белом списке оператора.

**Почему его не было в приложении.** `parseConfigEntries` явно выбрасывал все хосты с `type=xhttp`:
официальный sing-box (наш libbox на Android и в iOS-расширении) XHTTP не умеет, Remnawave такие
хосты в sing-box-рендер подписки тоже не отдаёт. Логика была «лучше скрыть, чем показать сервер,
который не подключится».

**Что изменено.** Фильтр снят (`services/vless_config_parser.dart`). Плагин нормализует `xhttp` в
sing-box `http`-транспорт (`vpn_config_parser.dart:909`) — это ближайший официальный эквивалент,
он стыкуется с Xray XHTTP в однопоточной форме (`stream-one`), но не со split-режимами
(`packet-up`/`stream-up`). Т.е. хост теперь **виден и выбираем**; если ядро с инбаундом не
договорится, это выглядит как обычный недоступный сервер, а не как пропавший.
...
**Не проверено / что дальше:** реальное подключение к этому хосту на устройстве. Если sing-box с ним
не договорится, правка на стороне панели (нужен доступ к Remnawave):
1. поднять на том же «белом» адресе/SNI хост-двойник на **gRPC** или **WebSocket/httpupgrade** —
   обход глушилок держится на адресе, а не на XHTTP; приложение подхватит его без релиза;
2. либо перевести XHTTP-инбаунд в режим `stream-one` (он проходит через nginx `grpc_pass` /
   Cloudflare gRPC) — с ним sing-box'овый `http` совместим ближе всего.
