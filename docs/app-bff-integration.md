...
274 теста приложения (+18), 212 плагина (+7), `flutter analyze` чист на обоих. **На устройстве
не проверено ничего.** Приёмка — `docs/release-test-checklist.md` §2d, 12 пунктов (TA1–TA13);
TA13 закрывается глазами, потому что `SettingsScreen` для рендера в тесте требует живую сессию
и мок сети.
