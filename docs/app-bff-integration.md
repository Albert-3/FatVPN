...
**Вывод: наш клиент к этому хосту подключиться не может.** Официальный sing-box не реализует XHTTP,
а `http`-транспорт, в который плагин ремапит xhttp, умеет только один поток — разделённый аплинк
`DELETE`'ом с padding'ом он не изобразит. Поэтому фильтр `type=xhttp` в
`services/vless_config_parser.dart` **оставлен**: хост, который всегда падает, пингуется нормально
