...
# ✅ ПЕРЕЕЗД ВЫПОЛНЕН 2026-08-06 — и что осталось после него

Всё, что выше, — план. Ниже — что фактически сделано и какие хвосты остались.
**Ни один пункт из «Осталось» не выполнен.**

## Что сделано (в этом порядке, за один заход)

| Шаг | Результат |
|---|---|
| Разведка `.29` | Там прод-бот `@Fat_VPN_bot` и системный nginx с живым платёжным доменом `pay.fatvnv.space`. Свободно 4 ядра / 7 ГБ / 83 ГБ |
| Репозиторий и `.env` | Клон в `/opt/fatvpn-bff/repo`, `.env` перенесён **побайтно** (sha256 сошлись — критично из-за `TRIAL_DEVICE_KEY_SALT`) |
| `docker-compose.override.yml` | `ports: !override` → BFF слушает только `127.0.0.1:5030`. `caddy` на этой машине не запускается **никогда** |
| Сборка и запуск | `docker compose up -d postgres bff`; миграции создали 9 таблиц |
| Перенос базы | `pg_dump --clean` → restore; сверка: Tokens 147, Sessions 886, Trials 77, Accounts 5, Devices 77, TokenDevices 18, PairingCodes 5 — совпало |
| TLS | Действующий серт **перенесён** из тома Caddy старого сервера в `/etc/ssl/fatvpn/` (окно ошибок TLS = 0). Ключ сверен с сертификатом |
| nginx vhost | `/etc/nginx/sites-available/api.fatklyuchi.space`: 80→443, ACME-локация, six `location =` под `/privacy`, `/privacy/ru`, `/support`, проксирование на `127.0.0.1:5030`. `pay.fatvnv.space` не тронут |
| Проверка до переключения | Полный путь приложения через `--resolve` по HTTPS: вход, `/me`, `/servers` 200, `/config` 200, чужой `Host` → 400 |
| **Реальный IP клиента** | Доказано опытом: исчерпал лимит `/auth` со своей машины (21-й запрос → 429), запрос со старого сервера в ту же минуту → 404. Счётчики раздельные |
| Финальная синхронизация | Повторный дамп непосредственно перед переключением |
| ⛔ Переключение DNS | `api.fatklyuchi.space` → `95.85.248.29`, серое облако. Оба NS Cloudflare подтвердили |
| Прокси для старых сборок | `/opt/fatvpn-legacy-proxy/` на `87.121.221.229`, слушает `:5030`, шлёт на новый по HTTPS с `Host: api.fatklyuchi.space`. Старый `fatvpn-bff` и `fatvpn-caddy` остановлены |
| Тестовый бот | Его `BFF_URL` переведён с docker-имени на домен (оригинал `.pre-migration-20260806`), пересобран. Секрет принимается новым BFF (404 на несуществующий код, а не отказ в доступе) |
| Бэкапы и сторожа | `/opt/fatvpn-ops/` на новом сервере, cron `:17` и `:43`; сторож на панели перенаправлен на `.29`; на старом сервере бэкап BFF отключён (база там заморожена) |

## Осталось — в порядке, в котором это стоит делать

### 1. Прод-бот: перенести интеграцию с приложением ⬜

Самое крупное и единственное, что мешает называть переезд завершённым: **в
прод-боте интеграции с приложением нет вообще.** Пока её нет, пользователи
приложения обслуживаются тестовым ботом на старом сервере.

Что переносить — по `docs/bot-prod-migration-brief.md` (контракт из 3 вызовов,
чек-лист приёмки из 11 пунктов). Чего конкретно не хватает в проде:

- целиком отсутствуют `bot/api/fatvpn_bff_api.py` и `bot/services/pairing_state.py`;
- вставки в пяти файлах: `main_refactored.py` (297 расходящихся строк),
  `handlers/trial_handler.py` (279), `handlers/key_handlers.py` (99),
  `database/db_remnawave.py` (62), `handlers/callback_handlers.py` (58).

⛔ **Копировать файлы из тестового бота поверх боевого нельзя.** Коды разошлись
**в обе стороны**: в боевом есть то, чего нет в тестовом — реферальные коды в
`main_refactored.py`, выдача ссылки на подписку в `callback_handlers.py` и
`key_handlers.py`. Копирование поверх уничтожит эту функциональность у платящих
пользователей. Перенос только хирургический, кусками, с чтением каждого места.

Мелочь, которая сэкономит время: бот и BFF теперь на одной машине и уже в одной
docker-сети `fatvpn_default`, поэтому `BFF_URL = "http://fatvpn-bff:5030"`
менять **не надо** — он и так верен.

⚠️ В `/opt/FatVPN` два дерева кода: исполняется только `bot/` (см. Dockerfile,
`WORKDIR /app/bot`), а в корне лежат устаревшие дубликаты `core/`, `handlers/`,
`main_refactored.py`. Легко отредактировать не тот файл и не понять, почему
ничего не изменилось.

### 2. Переключить приложение на прод-бота ⬜

`app/lib/config/api_config.dart`: `telegramBotUsername = 'testfatvpnnbot'` →
`fat_vpn_bot`. Делать **только после пункта 1**, иначе пользователи попадут в
бота, который не умеет их связывать с приложением. Требует нового билда — то
есть доезжает до людей не мгновенно, а по мере обновления.

### 3. Включить ufw на новом сервере ⬜

Сейчас `Status: inactive` (на старом — активен). Правила: `22`, `80`, `443` и
**обязательно `4444`** — это платёжный вебхук, забыть его значит оборвать приём
платежей. Помнить: docker-опубликованные порты ufw всё равно не фильтрует, так
что защита `5030` держится только на привязке к `127.0.0.1`.

### 4. Автопродление сертификата ⬜ — до октября

Серт действует до **2026-10-27**, дальше продлевать нечему: он перенесён руками,
а certbot про него не знает. `certbot certonly --nginx -d api.fatklyuchi.space`
(именно `certonly` — плагин иначе перепишет наш vhost и потеряет статику
`/privacy` и `/support`). Теперь, когда DNS указывает сюда, HTTP-01 пройдёт.
⛔ Только Let's Encrypt: приложение пинит корневые ISRG, смена центра
сертификации положит связь у всех установленных сборок.

### 5. Ротация секретов ⬜

Переезд их не менял, но вскрыл:

- секреты бота лежат **в исходниках** (`bot/core/config.py`: `TOKEN`,
  `PAYMENTS_TOKEN`, `REMNAWAVE_TOKEN`, `LOGIN_DATA`; `BOT_SECRET` захардкожен
  как fallback в `bot/api/fatvpn_bff_api.py`);
- в `git remote` репозитория бота на сервере — **токен GitHub открытым текстом**;
- архивы `bff-stack-*.tar.gz` и `bot-*.tar.gz` с секретами уезжают в Telegram
  каждые 6 часов;
- пароль Postgres не менялся с разработки (`fatvpn_dev`, дефолт из compose).

### 6. Погасить старое — не раньше чем через неделю ⬜

`87.121.221.229` держит: прокси для старых сборок (нужен, пока такие сборки
живы), тестового бота, и **замороженную базу как точку отката**. Ничего из этого
не удалять до тех пор, пока переезд не подтверждён неделей работы. Старые
контейнеры `fatvpn-bff` и `fatvpn-caddy` **не запускать** — они писали бы в
устаревшую базу.

### 7. Наблюдение ⬜

Первые сутки смотреть: приходят ли новые сессии в новую базу, нет ли всплеска
429 (рейт-лимит), не сыплются ли 502 от панели, и приходят ли почасовые бэкапы.
