...
| [improvement-plan-ios.md](improvement-plan-ios.md) | iOS (`app/ios`, PacketTunnel NE) | 3 critical, 7 high, 12 medium, 8 low | 🟡 закрыт в коде и **собирается** (Codemagic build 136 зелёный), но **на устройстве не проверено ничего**; 2.1 и 2.2 частично, четыре находки аудита оказались неверны, вычитка добавила 14 открытых пунктов (V17–V30); статус по каждой — в шапке документа |
