...
✅ **Все 14 пунктов вычитки (V17–V30) закрыты в коде 2026-07-29** — шесть коммитом
`9e23c11`, оставшиеся восемь `9b1c62a`. Из раздела «Вычитка ремедиации» открытым не
осталось ничего; построчный статус — в его таблице. «Закрыт в коде» здесь значит ровно
то же, что и везде в этом документе: Dart под хост-тестами, Swift проверен только
компилятором, поведение — ничем.

> ⚠️ **Абзац ниже устарел — сверено 2026-08-03.** Подпись трёх bundle id починена,
> `ios-release` с тех пор зелёный многократно: TestFlight **205** (`codesign` подтвердил
> App Group и в `Runner.app`, и в расширении экспортированного `.ipa`), **206**, **207**
> и дальше до **233**. Сверх того на iOS появились **поведенческие** данные, первые с
> 2026-07-24: фикс клоббера сессии (сборки 188/189) подтверждён живым пользователем
> 2026-08-01, нажатие виджета — на iPhone 11 / iOS 17.6.1 (build 207). ⚠️ Ни одна из
> **находок этого документа** ими не проверена: сессия и виджет — не туннель.
> Виджетная история с тех пор ушла дальше: App Intent удалён целиком 2026-08-03, нажатие
> стало ссылкой на всех версиях iOS (`docs/home-widgets-spec.md`).

⛔ **Codemagic `ios-release` красный с 2026-07-28.** Сборки 142–145 падали на подписи
виджета (`Signing for "FatVpnWidget" requires a development team`): профиль для
`com.fatvpn.fatvpnApp.FatVpnWidget` фетч по bundle id не приносил, даже когда он уже
лежал в портале активным. Фикс в `master` (`0a5890c`: берём **все** активные App
Store-профили команды через `app-store-connect profiles list --save`, плюс предархивный
шаг, проверяющий наличие профиля для всех трёх bundle id). **Зелёного прогона после
фикса не было ни одного.** Номер сборки считается как `git rev-list --count HEAD` —
на 2026-07-30 следующий будет **174**; `triggering:` в `codemagic.yaml` нет, запускать
вручную.
...
### 🟠 2.2 Clash API слушает loopback **без `secret`** — любое приложение управляет VPN — ✅ Обе половины сделаны (секрет 2026-07-28, порт 2026-08-02); на устройстве не проверено
...
>
> ✅ **Рандомизация порта — сделано 2026-08-02.** `VpnController._pickClashApiPort()`
> просит свободный порт у ОС (`ServerSocket.bind(loopbackIPv4, 0)`) на **каждый** старт
> туннеля и передаёт его через `buildFeatureSettings(clashApiPort:)` в
> `MiscOptions.clashApiPort`, то есть в `external_controller` конфига. Порт **не**
> берётся случайным из диапазона: занятый порт означал бы, что ядро не стартует, —
> отпечаток обменяли бы на «VPN иногда не подключается».
>
> Через start options ничего передавать не понадобилось: `TunnelHealthWatchdog.swift:468`
> и `VpnTunnelHealthProbe.kt:179` и раньше читали `external_controller` **из конфига**,
> а единственным местом с константой был `vpn_controller._singboxApiBase`. Теперь это
> геттер по сохранённому `vpn_clash_api_port`; отсутствие записи означает «туннель поднят
> прошлой сборкой» и опрос идёт на 16756. Константа `16756` осталась только как дефолт
> `MiscOptions` (её держат тесты плагина) и как этот фолбэк.
>
> Тесты: `app/test/audit_clash_secret_test.dart` — «не well-known порт» и «на каждый
> старт новый». ⚠️ **На устройстве не проверено**, и остаётся окно между `close()` и
> стартом ядра, в которое порт может занять кто-то ещё.
>
...
### Найдено и отложено — список работ, **закрытый 2026-07-29**

> На момент вычитки (2026-07-28) эти 14 пунктов были отложены; все они закрыты в коде
> назавтра, коммитами `9e23c11` и `9b1c62a`. Колонка «Почему не сделано сейчас» ниже
> переписана в «чем закрыто» — исходная формулировка каждой находки сохранена слева.
> На устройстве по-прежнему не проверен ни один.
...
| **V18** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `ExtensionPlatformInterface.swift` | Гонка на `nwMonitor`/`networkSettings`: пишутся из Go-потоков, читаются с очереди watchdog'а через `hasUsableUpstream`/`isUpstreamExpensive` — **обоих читателей добавила эта же ремедиация** | Заведена очередь `fatvpn.packet-tunnel.platform-state` по образцу 1.5: оба поля — за sync-аксессорами, все читатели/писатели идут через них |
| **V19** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `PacketTunnelProvider.swift` | `lazy var healthWatchdog` / `platformInterface` не потокобезопасны; точки первого касания лежат на четырёх разных потоках | `lazy` убраны: оба создаются один раз в `override init()`, до того как хоть один другой поток может держать ссылку на провайдер — потокобезопасно по построению |
| **V20** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `PacketTunnelProvider.swift` | **TTL снапшота 7 суток конфликтует с on-demand.** Метка времени пишется только при успешном старте; сессия живёт 90 дней | Живая сессия теперь освежает метку каждые 6 часов (`startSnapshotRefresh`, DispatchSourceTimer, гасится в `stopTunnel`): TTL стал значить «сессия *мертва* неделю», а не «сессия *жила* неделю» |
| **V21** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `SingboxMmPlugin.restartVpn` | Таймаут `awaitDisconnected` (10 с) трактовался как успех | `awaitDisconnected`/`stopTunnel` теперь отвечают флагом «реально ли легло»; `restartVpn` на таймауте отвечает `FlutterError(RESTART_TIMEOUT)` вместо старта поверх `.disconnecting` (который iOS молча роняет) — ошибка доходит до Dart, где неудавшийся свитч обрабатывается. Простой `stopVpn` флаг сознательно игнорирует: стоп без ожидающего старта — всё ещё стоп |
| **V22** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `ExtensionPlatformInterface.clearDNSCache` | `reasserting` снимался **только если коллбэк пришёл** | Однократный `finish` (сериализован очередью) вызывается либо коллбэком, либо 15-секундным таймаутом — кто первый; `tunnel` в обоих замыканиях захвачен слабо |
| **V23** 🟠 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `PacketTunnelProvider.swift` | `harden(stderrURL)` вызывался сразу после `LibboxRedirectStderr` | Файл теперь создаём сами **до** передачи пути в Go (`FileManager.createFile`, если отсутствует) и `harden` вызывается и до, и после редиректа — атрибуты ложатся на реальный inode, ленивое создание со стороны libbox становится дозаписью в уже защищённый файл. Проверять на T24 |
| **V24** 🟡 ✅ **закрыт в коде (2026-07-29, не проверен на устройстве)** | `connection_settings_controller.dart` | Снятие галки `autoReconnect`/`killSwitch` доходило до профиля **только при следующем ручном коннекте** | Новый `VpnController.applyTunnelPreferences()` пушит префы в профиль напрямую; `HomeScreen._onConnSettingsChanged` теперь различает настройки: флип префов → немедленный пуш **без** реконнекта и **независимо от `_isActive`** (снятая галка при лежащем туннеле — тот самый jetsam-сценарий), правка конфига → прежний реконнект с дебаунсом. Тесты: `audit_v24_preferences_flip_test.dart` (3 шт.) |
| **V25** 🟡 ✅ **закрыт в коде (2026-07-29, Swift — не компилировался локально)** | `PacketTunnelProvider.swift` | Строки `discarding expired / vN start options snapshot` писались **до** создания командного сервера | `writeMessage` без сервера паркует строки (кап 50), `flushPendingLogLines()` реплеит их с префиксом `(deferred)` сразу после `commandServer.start()` — след для T16 появляется в логе |
| **V26** 🟡 ✅ **закрыт в коде (2026-07-29; Dart — под тестами, Swift не компилировался локально)** | `redactSecrets` (оба Swift + Dart) | base64-строки ≥16 символов из рецепта **2.4** не реализованы; голый IPv4 без порта и **любой IPv6** не редактируются; `ss://<base64>@host` со слэшем внутри не ловится | Во всех трёх редакторах добавлены пасс-фильтры: hex ≥32 → `<redacted-hex>`; голый IPv4 → `<redacted-ip>` (loopback/0.0.0.0 сохраняются); IPv6-кандидаты с фильтром «`::` или ≥3 двоеточий» (таймстампы `12:00:00` выживают); base64-раны ≥20 → `<redacted-blob>` по эвристике «`+`/`=` или цифры+обе регистра» (тэги `outbound/shadowsocks` выживают); `://<cred-со-слэшем>@` ловится отдельным правилом. Dart: +8 тестов в `ios_audit_diagnostics_sanitizer_test.dart` (53 зелёные). Swift-копии зеркальны, но проверятся только сборкой Codemagic |
| **V27** 🟡 ✅ **закрыт в коде (2026-07-29, не проверен на устройстве)** | `PluginMethodDispatcher.kt` | `clearPersistedState` на Android — тихий no-op (`notImplemented`, глотается) | Реализован по всей цепочке (`PluginMethods.CLEAR_PERSISTED_STATE` → dispatcher → `PluginConfigOperations.clearPersistedState`): удаляются `active-config.json` и `active-xray-config.json` (+ их `.tmp`), `stderr.log`, и очищается снапшот `signbox_mm_runtime` (его `error` цитирует stderr, `configPath` указывает на стираемый конфиг). Best-effort по каждому артефакту, сам вызов не падает. Проверка на устройстве — T24-аналог для Android: после логаута в `files/` нет конфигов и stderr.log |
| **V28** 🟡 ✅ **закрыт признанием (2026-07-29)** | `traffic_throttle_policy.dart` | **Адаптивный MTU на iOS фактически выключен**: дефолт 1280, кандидаты лестницы 1400…1320, понижаться некуда | Выбран вариант «признать в тексте», потому что «расширить кандидаты вниз» некорректен: пол лестницы — 1280, минимум IPv6, а TUN на iOS теперь несёт inet6-адрес (1.7); кандидаты <1280 отбрасывает и `_resolveMtuCandidatesInternal`, и assert в самой политике. Инертность на iOS — by design, задокументирована док-комментарием у `mtuProbeCandidates`; лестница тюнит только конфигурации с MTU выше пола (Android-пресеты, явный `tunMtu`) |
| **V29** 🟡 ✅ **закрыт (2026-07-29)** | тесты | `ios_audit_memory_profile_test.dart` целиком и группа §2.2 в `clash_api_secret_test.dart` пиннят код, который был в `master` **до** ремедиации | Появился сквозной `audit_v29_feature_settings_end_to_end_test.dart`: реальный путь `ConnectionSettingsController.buildFeatureSettings() → SingboxConfigBuilder` под iOS/Android-оверрайдами, ассерты на §3.2 (stack=system), §3.3 (cache_file off), §3.4 (MTU 1280) и §2.2 (секрет в `clash_api`) по эмитированному JSON — откат любой из ремедиаций красит его. Клэмп release-лога вынесен в публичный чистый `SingboxConfigBuilder.clampLogLevel(isRelease:)` и покрыт (включая регистр). Старые плацебо-тесты оставлены как есть — вреда от них нет, доказательство теперь не на них |
| **V30** 🟡 ✅ **закрыт (2026-07-29)** | CI | `pubspec.lock` не в git ни в `app/`, ни в `app/packages/singbox_mm/` | Оба lock-файла закоммичены (`.gitignore` поправлены с объяснением: приложение — не библиотека, дефолт pub для library тут не про нас; singbox_mm собирается только в составе приложения). Следующая Codemagic-сборка резолвит зависимости из lock |
...
| ~~Все пункты вычитки~~ | V17–V30 | ✅ **Закрыты в коде 2026-07-29** (`9e23c11` — шесть, `9b1c62a` — восемь): logout/402 teardown (V17), гонки в расширении (V18, V19), TTL снапшота (V20), таймауты (V21, V22), `harden` до редиректа (V23), пуш префов без реконнекта (V24), парковка логов (V25), санитайзер (V26), `clearPersistedState` на Android (V27), инертность MTU (V28 — закрыт признанием), сквозной тест вместо плацебо (V29), lock-файлы в git (V30). Построчно — в таблице раздела «Вычитка ремедиации». **Ни один не проверен на устройстве:** V17 → T15, V23 → T24, V24/V27 → device-приёмка |

---

## Рекомендуемый порядок работ

> **Статус на 2026-07-30:** все четыре спринта отработаны в коде, вычитка (V17–V30)
> закрыта 2026-07-29. Осталось то, что перечислено в «Что осталось открытым»,
> ~~зелёная `ios-release` (красная с 142-й)~~ и **вся** проверка на устройстве.
> ✏️ **2026-08-03:** зелёная сборка давно есть (TestFlight 205 → 233), так что из этих
> трёх остатков сборка отпала. Проверка на устройстве — не отпала ни на пункт: то, что
> на устройстве запускали, проверяло сессию и виджет, а не туннель.
