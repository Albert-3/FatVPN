...
задач. Все правки — в рабочем дереве, **не закоммичены**.

⚠️ **Главная оговорка: Swift здесь не компилировался.** В окружении нет Xcode, поэтому ни
одна из четырёх изменённых Swift-единиц (`PacketTunnelProvider.swift`,
`ExtensionPlatformInterface.swift`, `TunnelHealthWatchdog.swift`,
`SingboxMmPlugin.swift`) не проверена даже на сборку. Максимальный честный статус любой
Swift-находки — «исправлено в коде», и это **не** то же самое, что «исправлено».

⚠️ **На устройстве не проверено ничего.** Приёмка — блок **T-iOS** раздела 2a в
`docs/release-test-checklist.md` (T12–T16, T23–T25).
...
| Метка | Что значит |
|---|---|
| 🟡 **код (Swift)** | правка есть в рабочем дереве; **не компилировалась**, на устройстве не проверялась |
...
| 🟡 **CI** | правка в `codemagic.yaml`; проверится первым же прогоном воркфлоу |
...
### 🔴 1.1 `serviceStop()` останавливает ядро, но не туннель — полная потеря связи при «connected» — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🔴 1.2 Окно утечки трафика в `clearDNSCache()` — срабатывает ровно при смене сети — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟠 1.4 `runBlocking` без таймаута → детерминированное зависание старта — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟠 1.5 Гонка данных на `tunnelOptions` — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟠 1.8 DNS применяется «по возможности» и без `matchDomains` — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟠 1.9 `fatalError` в расширении при недоступности App Group — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟡 1.10 `stopVpn`/`restartVpn`: гонка с `.disconnecting` и «отравление» `vpnManager` — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟡 1.11 `pingServer`: гонка данных на `payload` + утечка `NWConnection` — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟡 1.12 Статистика трафика на iOS всегда нулевая, но таймер тикает каждую секунду — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟡 1.13 `sleep()` может оставить ядро в паузе навсегда — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟢 1.15 `handleAppMessage` не проверяет тип сообщения — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟡 2.3 `external_controller` из конфига не валидируется → исходящий HTTP из расширения — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟠 3.1 Чтение `stderr.log` целиком в память ровно тогда, когда памяти уже нет — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟢 3.6 105 секунд до первого восстановления — 🟡 Исправлено в коде (Swift не компилировался)
...
### 🟢 3.7 `URLSession` в watchdog'е без ограничения кеша — 🟡 Исправлено в коде (Swift не компилировался)
...
Правки аудита читались повторно, построчно, всеми четырьмя слоями. Swift по-прежнему
**не компилировался**: часть находок ниже — это то, что компилятор бы и не поймал.
...
|---|---|---|---|
| **V17** 🔴 | `auth_controller.dart`, `main.dart`, `home_screen.dart` | **Логаут по 401 и истечение подписки (402) обходят `forgetPersistedTunnelState()`.** При `!subscriptionActive` `HomeScreen` снимается с дерева, `onSessionDropped` обнуляется, а автоматический `signOut()` из `_doRefresh` выполняется уже без него. На 402 туннель вдобавок **не останавливается вовсе** | Требует правки жизненного цикла экранов, а не одной функции. **Прямо подрывает 1.6 и 2.1:** на iOS с включённым on-demand ОС продолжит поднимать туннель из `start_options.plist` на кредах, которые панель могла отозвать. Докстринг `vpn_controller.dart` обещает закрыть именно этот случай («after a power-off, a logout or an expired subscription») — и не закрывает |
...
> **Состояние критерия на 2026-07-28: не выполнен ни в одной части.**
>
> - ⬜ **Сборка в Codemagic не прогонялась.** Xcode в окружении нет, Swift не
>   компилировался. Первый прогон `ios-release` заодно проверит три новых шага сразу:
>   доступность `flutter: 3.44.4` на раннере, гейт `flutter analyze && flutter test` и
>   `codesign -d --entitlements` (последний может оказаться первым, кто покажет, что у
>   App ID Runner'а нет capability App Groups — см. **1.9**, **2.5**).
