...
|---|---|---|
| Скриншоты 6.9″ (и 6.5″, если версия старая) | ASC не даёт сабмитить версию без набора | Вариант А: попросить владельца iPhone 15 снять 5 экранов из §5 (RU), быстрее всего. Вариант Б: расширить `ios-simulator` workflow в Codemagic — поднять iPhone 16 Pro Max, нащёлкать `simctl io screenshot`, забрать артефактами |
