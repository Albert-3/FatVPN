...
## Доработка 3: код для приложения на **всех** входах (2026-08-03)

Заказчик сообщил симптом: купил ключ — на экране «🔥 · Доступ успешно получен» есть
ссылка на подписку и кнопки платформ, а кода для приложения нет. Причина оказалась
глубже симптома: **покупка вообще не ходила в BFF** — `handle_tarif_purchase`
(`bot/handlers/callback_handlers.py`) после оплаты не звал ни `register_short_token`,
ни `upsert_subscription`. Соседние ветки делают это давно («Ваш ключ», «Поменять
ключ»), то есть ровно на пути «заплатил и хочу пользоваться» приложения как бы не
существовало, а код находился только через «Мои ключи». То же самое было на экране
пробного ключа (`trial_handler.py`).

Что сделано в обеих ветках (Точки 1 и 2 этой доработки):

1. срок берётся из панели (`gettime(full_key)`) с фолбэком на «сегодня + дни» —
   продление существующего ключа даёт дату дальше, чем арифметика по тарифу;
2. `register_short_token(short_uuid, expires_at_ms, user_id)` — сам код;
3. код кладётся в `key_handler.key_token_map[full_key]`, чтобы экран ключа показал
   **этот же** код, а не выпустил ещё один;
4. `upsert_subscription(..., key_code=short_token)` **без** `make_active` — сознательно:
   BFF применит это к аккаунту без активного ключа или с истёкшим (обычный случай
   «подписка кончилась → купил новую»), а переезд с живого ключа остаётся явным
   выбором пользователя. `make_active=True` — одно слово, но это продуктовое решение;
5. весь блок в `try`: деньги списаны и ключ выпущен, поэтому сбой здесь не имеет права
   оставить пользователя без экрана — рисуется прежний экран, просто без кода.

Плюс одна строка проводки в `main_refactored.py` — `trial_handler.key_handler =
key_handler`, рядом с такой же для `callback_handler`; без неё «Мои ключи» выпустили бы
для пробного ключа второй код.

**Проверено на живом стеке, но не покупкой:** из контейнера бота вызваны те же две
функции с синтетическим `telegramUserId` — код выпустился, в `Accounts` появились
`CurrentSubscriptionId` и `CurrentKeyCode`, в `Tokens` — код с `AccountId`; тестовые
строки удалены. Непроверенным остаётся сам экран после реальной оплаты (нужен баланс).
Смотреть при первой же покупке: `docker logs -f fatvpn-bot | grep -i "код приложения"` —
эта строка появляется **только** когда выдача кода упала.

Оригиналы рядом с файлами: `callback_handlers.py.pre-buycode`,
`trial_handler.py.pre-buycode`, `main_refactored.py.pre-buycode`. Полный разбор —
`docs/app-bff-integration.md`, раздел «Покупка ключа в боте не выдавала код для
приложения».

⚠️ Не путать с триалом **приложения** (`POST /trial` в BFF, со своей защитой от
переустановок): здесь речь о пробном ключе панели, который выдаёт бот.
