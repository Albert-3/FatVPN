/// The certificate authorities the app is willing to talk to the BFF through.
///
/// Everything the app sends the BFF — the refresh token that is good for 90
/// days, the subscription id that is itself a bearer credential for the config —
/// is protected by TLS alone. Out of the box that means any of the ~150 CAs in
/// the device's trust store may vouch for `api.fatklyuchi.space`, plus, on iOS,
/// any CA someone talked the user into installing as a profile. This bundle
/// narrows that to Let's Encrypt, which is the only CA that has ever issued for
/// this domain.
///
/// **What is pinned is the trust anchor, not the leaf.** A pin on the server's
/// own key is stronger and would brick every installation the first time Caddy
/// renews: certmagic generates a fresh key per renewal, roughly every 60 days,
/// and there is no remote switch to turn a bad pin off — the fix would be a
/// store release, which is days on iOS. Anchors move on a scale of years and
/// with public notice, and all four current ISRG roots are here, so a renewal, a
/// new intermediate, or a switch between the RSA and ECDSA chain all pass
/// unnoticed.
///
/// | Root | Key | Expires | SHA-256 |
/// |---|---|---|---|
/// | ISRG Root X1 | RSA | 2035-06-04 | `96:BC:EC:06:...:BD:DF:08:C6` |
/// | ISRG Root X2 | ECDSA | 2040-09-17 | `69:72:9B:8E:...:3C:CB:14:70` |
/// | ISRG Root YE | ECDSA | 2045-09-02 | `E1:4F:FC:AD:...:AA:5D:56:66` |
/// | ISRG Root YR | RSA | 2045-09-02 | `E5:7B:7E:6F:...:EB:F4:A8:6F` |
///
/// Downloaded from `letsencrypt.org/certs/` on 2026-08-02 and checked two ways:
/// X1's and X2's fingerprints against their long-published values, and the live
/// chain of `api.fatklyuchi.space` against this bundle *with the system trust
/// store switched off* (`openssl s_client -CAfile … -no-CAstore -no-CApath
/// -verify_return_error` → `Verify return code: 0`). Today the server serves
/// leaf ← `YE2` ← `Root YE` ← `ISRG Root X2`, i.e. two of these four are on the
/// path at once. The full fingerprints are asserted by
/// `test/ca_pinning_test.dart`, so this file cannot be edited quietly.
///
/// ⚠️ **Operational contract.** Changing the certificate authority of
/// `api.fatklyuchi.space` — moving to ZeroSSL, to a Cloudflare origin
/// certificate, to anything not signed by ISRG — takes every installed build
/// offline until a new one ships. A new CA has to be added here, released, and
/// waited out *before* the server changes. Renewals are safe and need nothing.
library;

/// PEM bundle of the four ISRG roots, verbatim as published. Concatenated
/// certificates; nothing else may go in this string — the parser is fed it whole.
const letsEncryptRootsPem = r'''
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

/q4AaOeMSQ+2b1tbFfLn
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
''';
