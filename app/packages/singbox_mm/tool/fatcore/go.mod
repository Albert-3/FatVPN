// One Go module holding both cores the app needs.
//
// They cannot be two separate mobile libraries. A gomobile binding always
// ships the `go.Seq` support classes under that exact name, so two bindings in
// one APK collide at dex time; on iOS the two static Go archives collide even
// harder, at link time, over the runtime symbols they each define. Building a
// single library that binds sing-box's libbox alongside a small Xray wrapper
// gives one Go runtime, one binding, and one artifact per platform.
//
// sing-box is v1.13.14 rather than the v1.13.11 the old fetch scripts pinned:
// this graph resolves a newer sing-tun, and 1.13.11 does not compile against
// its interface-monitor API. A patch release inside the same minor line, but
// worth knowing it moved with this change.
module github.com/fatvpn/fatcore
...
require (
	...
	github.com/xtls/libxray v0.0.0-20260723104709-6f806fc0a442
)
